...
import com.shardingSphere.demo.exception.IllegalFileException;
import com.shardingSphere.demo.mapper.PDFInfoMapper;
import com.shardingSphere.demo.model.PDFInfo;
import com.shardingSphere.demo.model.PDFStatus;
...
public class PDFServiceImpl implements PDFService {
    ...
    private final PDFInfoMapper pdfInfoMapper;
    ...
    public PDFServiceImpl(CosUtil cosUtil, PDFInfoMapper pdfInfoMapper) {
        this.pdfInfoMapper = pdfInfoMapper;
...
//            pdfInfo.setId(Constant.SELF_INCRMENT_ID.addAndGet(1));
            pdfInfo.setMd5(md5);
            pdfInfo.setFileName(file.getOriginalFilename());
            pdfInfo.setUserId(userId);
            pdfInfo.setStatus(PDFStatus.PROCESSING);
            pdfInfoMapper.insert(pdfInfo);
            // 将原PDF存储到COS
            String url=cosUtil.uploadStream(inputStream, md5, file.getSize(), transfer);
            pdfInfo.setUrl(url);
            pdfInfoMapper.updateById(pdfInfo);
            // 对单个文件进行拆分与存储
            singleFileProcess(file, size, page, prepare,userId);
        }
        byte[] zip = ZipUtil.createZip(prepare);
        cosUtil.shutdownTransferManager(transfer);
    ...
    private void singleFileProcess(MultipartFile file, Long size, Integer page, HashMap<String, byte[]> prepare,Long userId) {
        ...
        try (PDDocument document = Loader.loadPDF(file.getBytes())) {
            ...
            if (page != null) {
                ...
                throw new IllegalFileException(Constant.SIZE_AND_PAGE_CANNOT_BE_NULL);
            }
        } catch (IOException e) {
            throw new FileException();
        }
        ...
        if (splitPages.size() == 1) {
            ...
            // 存入分页后文档的名称与文档二进制数据,存储分页后的所有pdf
            for (int i = 0; i < splitPages.size(); i++) {
                byte[] bytes = splitPages.get(i);
                splitFileName = Objects.requireNonNull(file.getOriginalFilename()).replaceAll(".pdf", "") + "_" + (i + 1) + ".pdf";
                prepare.put(splitFileName, bytes);
                // 创建md5
                String md5 = DigestUtils.md5DigestAsHex(splitPages.get(i));
                PDFInfo pdfInfo = new PDFInfo();
//                pdfInfo.setId(Constant.SELF_INCRMENT_ID.addAndGet(1));
                pdfInfo.setMd5(md5);
                pdfInfo.setFileName(splitFileName);
                pdfInfo.setUserId(userId);
                pdfInfo.setStatus(PDFStatus.PROCESSING);
                pdfInfoMapper.insert(pdfInfo);
                ...
                try (ByteArrayInputStream bais = new ByteArrayInputStream(splitPages.get(i))) {
                    url=cosUtil.uploadStream(bais, md5, splitPages.get(i).length, transfer);
                } catch (IOException e) {
                    throw new FileException();
                }
                if (url!=null) pdfInfo.setUrl(url);
                pdfInfo.setStatus(PDFStatus.SUCCESS);
                pdfInfoMapper.updateById(pdfInfo);
            }
        }
    }
